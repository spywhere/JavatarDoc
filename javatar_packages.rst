...
|J| Packages file is a JSON file. You can read more details about each key and value in Proto.javatar-packages located within |J|'s Java folder (can be accessed via *Preferences > Package Settings > Proto.|J|-packages* or via command palette by type ``Javatar Proto``).
...
Example of |J| Packages is located inside |J|'s Java folder (`PACKAGES_PATH/Javatar/Java`).